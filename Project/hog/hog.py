...
def boar_brawl(player_score, opponent_score):
    ...
    got_score =3 * abs(player_digit - opponent_digit)
    if got_score:
        return got_score
    else:
        return 1 
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    dice_score = 0
    if num_rolls == 0:
        dice_score = boar_brawl(player_score, opponent_score)
    else:
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    def strategy(score, oponent_score):
        return n
    return strategy
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    not_always = 0
    count = 0
    roll_time = strategy(player_score, opponent_score)
    while player_score < goal:
        opponent_score = 0
        while opponent_score < goal:
            if strategy(player_score, opponent_score) != roll_time:
                not_always += 1 
            count += 1
            opponent_score += 1
            """print ('DEBUG: srategy is',strategy(player_score, opponent_score))
            print ('DEBUG: rolltime is',roll_time)
            print('DEBUG: not_always is',not_always)
            print('DEBUG: count is',count)
            print('DEBUG: x is',player_score)
            print('DEBUG: y is',opponent_score)
            """
        player_score += 1
    if not_always:
        return False
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    """
    # BEGIN PROBLEM 9
    score_averaged_f = make_averaged(roll_dice, times_called)
    num = 0
    n_score_averaged = 0 
    while(num < 10):
        num += 1
        b_score_averaged = n_score_averaged 
        n_score_averaged = score_averaged_f(num, dice)
        if n_score_averaged > b_score_averaged:
            max_roll = num
    return max_roll
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    increase = sus_points(boar_brawl(score, opponent_score) + score) - score
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    """def min_num():
        score_averaged_f = make_averaged(roll_dice, times_called=1000)
        num = 0
        while(num < 10):
            num += 1
            score_averaged = score_averaged_f(num, six_sided)
            if score + score_averaged > GOAL:
                return num"""
